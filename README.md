...
- **🗓️ 스케줄 관리**: 시나리오를 주기적으로 실행하도록 등록/조회/활성화 관리
...
│   ├── ipc-handlers-sqlite.ts    # 시나리오/스케줄 IPC 핸들러
│   ├── database-sqlite.ts        # SQLite 데이터 계층
│   ├── scheduler-windows.ts      # Windows 작업 스케줄러 연동
│   ├── playwright-electron-recorder.ts  # Playwright 레코딩 엔진
│   └── playwright-electron-debug.ts     # Playwright 디버그 모드
├── src/                          # React 렌더러 프로세스
│   ├── app/                      # 라우트 페이지
│   │   ├── scenario/             # 시나리오 생성/편집
│   │   └── schedules/            # 스케줄 목록/등록/상세
│   ├── features/                 # 기능별 컴포넌트/서비스
│   ├── shared/                   # 공통 UI/유틸
...
- **scheduler-windows.ts**: Windows 작업 스케줄러(schtasks) 등록/활성화/삭제
...
### 4. 🗓️ 스케줄 관리
1. **스케줄 등록**: 시나리오 편집 화면 또는 "스케줄 관리 > 스케줄 등록"에서 주기 설정
2. **주기 설정**: 매일(`DAILY`), 매주(`WEEKLY`), 매달(`MONTHLY`) + 실행 시간 지정
3. **상태 관리**: 스케줄 목록/상세에서 활성화 토글, 필터, 새로고침
4. **이력 확인**: 스케줄 상세에서 실행 이력 및 실행 로그 확인

> 참고: macOS/Linux에서는 스케줄 정보를 저장하고 조회할 수 있으며, OS 작업 스케줄러 기반 자동 실행은 Windows 앱에서 동작합니다.
...
주요 테이블:
- `scenarios`: 시나리오 메타데이터 및 Playwright 코드
- `executions`: 수동/스케줄 실행 결과 상태(`RUNNING`, `SUCCESS`, `FAILURE`)
- `schedules`: 시나리오별 주기/시간/활성화 설정
- `schedule_runs`: 스케줄 트리거로 실행된 이력
